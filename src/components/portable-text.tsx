...
function renderBlock(block: Block): JSX.Element {
    ...
    return (
        <Switch fallback={<p>{children}</p>}>
            <Match when={block.style === "h1"}>
                <h1>{children}</h1>
            </Match>
            <Match when={block.style === "h2"}>
                <h2>{children}</h2>
            </Match>
            <Match when={block.style === "h3"}>
                <h3>{children}</h3>
            </Match>
            <Match when={block.style === "h4"}>
                <h4>{children}</h4>
            </Match>
            <Match when={block.style === "blockquote"}>
                <blockquote>{children}</blockquote>
            </Match>
            <Match when={block.style === "normal" || !block.style}>
                <p>{children}</p>
            </Match>
        </Switch>
    );
